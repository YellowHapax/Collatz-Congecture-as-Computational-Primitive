...
**Date:** June 2026
**Status:** Working Paper — Noetic Lab Series
...
The full trajectory is a product of 2×2 matrices, placing it in the natural habitat of tensor structure. Trajectories compose via matrix multiplication but do not add: the 3n+1 step mixes additive and multiplicative structure, breaking distributivity. This non-additivity is precisely why the conjecture is hard.
...
Conway (1972) proved that generalized Collatz-type maps — iterative affine rules selected by residue class — are Turing complete. The Collatz conjecture is therefore an instance of the halting problem. The process is computationally irreducible: no shortcut exists that predicts the trajectory's length or endpoint without running the computation. The conjecture asks whether a particular Turing-complete process always halts, and the answer is not known because, in general, such answers cannot be known.
...
The Collatz analog-digital interface maps onto the core architecture of the Memory as Baseline Deviation framework (Everett, 2024–2026).
...
The conjecture is unproven because the answer is not obvious. And the framework proposed here suggests a more precise formulation: communication can succeed whenever the trajectory is finite (the signal is reducible within the available compute budget and bandwidth), but the system cannot determine in advance whether a given trajectory will terminate. This is the halting problem applied to the interface between incommensurate scales.
...
The exponential kernel is not a modeling choice. It is the unique consequence of linear, first-order memory dissipation. The choice of kernel *is* the theory, and the theory here is that un-reinforced sensory traces decay at a rate proportional to their current activation — the simplest dissipative law consistent with biological constraints.
...
The same machine — iterative reduction between incommensurate scales — appears across domains:
...
The conjecture is one instance. The machine class is the asset.
...
*This paper identifies Collatz-type iterative reduction as the shared computational primitive underlying analog-to-digital conversion, acoustic temperament theory, and MBD communication dynamics. It classifies four mechanistically distinct failure modes of inter-baseline communication: the Wolf Interval, Truncation, Bandwidth Mismatch, and True Divergence. It derives a real-time temporal renormalization group flow for the cognitive baseline, establishing an exact stability bound on inter-parameter coupling via the Routh-Hurwitz criterion. It defines wisdom as the capacity to perform lossy compression at the fidelity appropriate to a given channel while retaining awareness of what the compression discards.*
